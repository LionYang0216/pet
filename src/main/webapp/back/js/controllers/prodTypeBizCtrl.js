//list ctrl
angular.module('backApp').controller('bizListCtrl', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, $http) {
	
	$scope.$on('$viewContentLoaded', function() {  				
		var bizListDataTable = new Datatable().init({
            src: $("#bizListDataTable"),
            dataTable: {
    			"language": dtLanguage,
    			...
    			"ajax" : {
    				"type" : "POST",
    				"url" : "../prodTypeBiz/getProdTypeBizList", // ajax
    				"contentType": "application/json; charset=utf-8",
    				"data": function (dtRequest) {
    					  dtRequest = $.extend({}, dtRequest, {condition:$scope.bizCondition}); // merge the form data into current Query
    			...
    			},
    			{
    				...
    				"render" : function(data, type, row, meta) {
    					var str = '<a class="btn default" href="#/prodTypeBiz/form/' + data + '">编辑</a>';
    					return str;
    				},
    				"targets" : 7,
    				'searchable' : false,
	...
	$scope.reloadTable = function(){
		var bizListDataTable= $('#bizListDataTable').DataTable();
		bizListDataTable.ajax.reload();
	}
...
//form ctrl
angular.module('backApp').controller('bizFormCtrl', ['$rootScope', '$scope','$stateParams','$state', 'settings', function($rootScope, $scope,$stateParams,$state, $http) {
	// page load: retrieve admin
	$scope.$on('$viewContentLoaded', function() { 
		if($stateParams.id > 0){ // is update
			var reqData= $stateParams;
			var url = "../prodTypeBiz/getProdTypeBiz";
			jsonGet(url, reqData, callback, true, false);
			
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					$scope.prodTypeBiz = result.data;
	...
	// update or add an prodTypeBiz
	$scope.toSaveProdTypeBiz = function(){

		var reqData = $scope.prodTypeBiz;
		...
		if($stateParams.id > 0){ // is update
			var url = "../prodTypeBiz/updateProdTypeBiz";
			jsonPost(url, reqData, callback, true, false);
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					toastr.success(result.msg, COMMON_LABEL_SUCCESS);
					$rootScope.$state.go('prodTypeBizList');
				} else {
					toastr.error(result.msg, COMMON_LABEL_ERROR);
				}
		    }
		}else{ // is add
			var url = "../prodTypeBiz/addProdTypeBiz";
			jsonPost(url, reqData, callback, true, false);
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					toastr.success(result.msg, COMMON_LABEL_SUCCESS);
	...
	// delete an prodTypeBiz
	$scope.toDeleteProdTypeBiz = function(){
		if($stateParams.id){
			bootbox.confirm("是否删除?", function (result) {
				if(result){
					var reqData= $stateParams;
					var url = "../prodTypeBiz/deleteProdTypeBiz";
					jsonGet(url, reqData, callback, true, false);
					function callback(result) {
						if (result.flag == RESULT_FLAG_SUCCESS) {
							toastr.success(result.msg, COMMON_LABEL_SUCCESS);

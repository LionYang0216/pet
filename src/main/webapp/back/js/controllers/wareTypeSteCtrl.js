//list ctrl
angular.module('backApp').controller('wareTypeSteListCtrl', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, $http) {
	
	$scope.$on('$viewContentLoaded', function() {  				
		var areListDataTable = new Datatable().init({
            src: $("#wareTypeSteListDataTable"),
            dataTable: {
    			"language": dtLanguage,
    			...
    			"ajax" : {
    				"type" : "POST",
    				"url" : "../wareTypeSte/getWareTypeSteList", // ajax
    				"contentType": "application/json; charset=utf-8",
    				"data": function (dtRequest) {
    					  dtRequest = $.extend({}, dtRequest, {condition:$scope.wareTypeSteCondition}); // merge the form data into current Query
    			...
    			},
    			{
    				...
    				"render" : function(data, type, row, meta) {
    					var str = '<a class="btn default" href="#/wareTypeSte/form/' + data + '">编辑</a>';
    					return str;
    				},
    				"targets" : 7,
    				'searchable' : false,
	...
	$scope.reloadTable = function(){
		var wareTypeSteListDataTable= $('#wareTypeSteListDataTable').DataTable();
		wareTypeSteListDataTable.ajax.reload();
	}
...
//form ctrl
angular.module('backApp').controller('wareTypeSteFormCtrl', ['$rootScope', '$scope','$stateParams','$state', 'settings', function($rootScope, $scope,$stateParams,$state, $http) {
	// page load: retrieve admin
	$scope.$on('$viewContentLoaded', function() { 
		if($stateParams.id > 0){ // is update
			var reqData= $stateParams;
			var url = "../wareTypeSte/getWareTypeSte";
			jsonGet(url, reqData, callback, true, false);
			
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					$scope.wareTypeSte = result.data;
	...
	// update or add an wareTypeSte
	$scope.toSaveWareTypeSte = function(){

		var reqData = $scope.wareTypeSte;
		...
		if($stateParams.id > 0){ // is update
			var url = "../wareTypeSte/updateWareTypeSte";
			jsonPost(url, reqData, callback, true, false);
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					toastr.success(result.msg, COMMON_LABEL_SUCCESS);
					$rootScope.$state.go('wareTypeSteList');
				} else {
					toastr.error(result.msg, COMMON_LABEL_ERROR);
				}
		    }
		}else{ // is add
			var url = "../wareTypeSte/addWareTypeSte";
			jsonPost(url, reqData, callback, true, false);
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					toastr.success(result.msg, COMMON_LABEL_SUCCESS);
	...
	// delete an wareTypeSte
	$scope.toDeleteWareTypeSte = function(){
		if($stateParams.id){
			bootbox.confirm("是否删除?", function (result) {
				if(result){
					var reqData= $stateParams;
					var url = "../wareTypeSte/deleteWareTypeSte";
					jsonGet(url, reqData, callback, true, false);
					function callback(result) {
						if (result.flag == RESULT_FLAG_SUCCESS) {
							toastr.success(result.msg, COMMON_LABEL_SUCCESS);

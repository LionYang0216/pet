//list ctrl
angular.module('backApp').controller('widListCtrl', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, $http) {
	
	$scope.$on('$viewContentLoaded', function() {  				
		var widListDataTable = new Datatable().init({
            src: $("#widListDataTable"),
            dataTable: {
    			"language": dtLanguage,
    			...
    			"ajax" : {
    				"type" : "POST",
    				"url" : "../prodTypeWid/getProdTypeWidList", // ajax
    				"contentType": "application/json; charset=utf-8",
    				"data": function (dtRequest) {
    					  dtRequest = $.extend({}, dtRequest, {condition:$scope.widCondition}); // merge the form data into current Query
    			...
    			},
    			{
    				...
    				"render" : function(data, type, row, meta) {
    					var str = '<a class="btn default" href="#/prodTypeWid/form/' + data + '">编辑</a>';
    					return str;
    				},
    				"targets" : 7,
    				'searchable' : false,
	...
	$scope.reloadTable = function(){
		var widListDataTable= $('#widListDataTable').DataTable();
		widListDataTable.ajax.reload();
	}
...
//form ctrl
angular.module('backApp').controller('widFormCtrl', ['$rootScope', '$scope','$stateParams','$state', 'settings', function($rootScope, $scope,$stateParams,$state, $http) {
	// page load: retrieve admin
	$scope.$on('$viewContentLoaded', function() { 
		if($stateParams.id > 0){ // is update
			var reqData= $stateParams;
			var url = "../prodTypeWid/getProdTypeWid";
			jsonGet(url, reqData, callback, true, false);
			
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					$scope.prodTypeWid = result.data;
	...
	// update or add an prodTypeWid
	$scope.toSaveProdTypeWid = function(){

		var reqData = $scope.prodTypeWid;
		...
		if($stateParams.id > 0){ // is update
			var url = "../prodTypeWid/updateProdTypeWid";
			jsonPost(url, reqData, callback, true, false);
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					toastr.success(result.msg, COMMON_LABEL_SUCCESS);
					$rootScope.$state.go('prodTypeWidList');
				} else {
					toastr.error(result.msg, COMMON_LABEL_ERROR);
				}
		    }
		}else{ // is add
			var url = "../prodTypeWid/addProdTypeWid";
			jsonPost(url, reqData, callback, true, false);
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					toastr.success(result.msg, COMMON_LABEL_SUCCESS);
	...
	// delete an prodTypeWid
	$scope.toDeleteProdTypeWid = function(){
		if($stateParams.id){
			bootbox.confirm("是否删除?", function (result) {
				if(result){
					var reqData= $stateParams;
					var url = "../prodTypeWid/deleteProdTypeWid";
					jsonGet(url, reqData, callback, true, false);
					function callback(result) {
						if (result.flag == RESULT_FLAG_SUCCESS) {
							toastr.success(result.msg, COMMON_LABEL_SUCCESS);

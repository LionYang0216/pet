//list ctrl
angular.module('backApp').controller('areListCtrl', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, $http) {
	
	$scope.$on('$viewContentLoaded', function() {  				
		var areListDataTable = new Datatable().init({
            src: $("#areListDataTable"),
            dataTable: {
    			"language": dtLanguage,
    			...
    			"ajax" : {
    				"type" : "POST",
    				"url" : "../prodTypeAre/getProdTypeAreList", // ajax
    				"contentType": "application/json; charset=utf-8",
    				"data": function (dtRequest) {
    					  dtRequest = $.extend({}, dtRequest, {condition:$scope.areCondition}); // merge the form data into current Query
    			...
    			},
    			{
    				...
    				"render" : function(data, type, row, meta) {
    					var str = '<a class="btn default" href="#/prodTypeAre/form/' + data + '">编辑</a>';
    					return str;
    				},
    				"targets" : 7,
    				'searchable' : false,
	...
	$scope.reloadTable = function(){
		var areListDataTable= $('#areListDataTable').DataTable();
		areListDataTable.ajax.reload();
	}
...
//form ctrl
angular.module('backApp').controller('areFormCtrl', ['$rootScope', '$scope','$stateParams','$state', 'settings', function($rootScope, $scope,$stateParams,$state, $http) {
	// page load: retrieve admin
	$scope.$on('$viewContentLoaded', function() { 
		if($stateParams.id > 0){ // is update
			var reqData= $stateParams;
			var url = "../prodTypeAre/getProdTypeAre";
			jsonGet(url, reqData, callback, true, false);
			
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					$scope.prodTypeAre = result.data;
	...
	// update or add an prodTypeAre
	$scope.toSaveProdTypeAre = function(){

		var reqData = $scope.prodTypeAre;
		...
		if($stateParams.id > 0){ // is update
			var url = "../prodTypeAre/updateProdTypeAre";
			jsonPost(url, reqData, callback, true, false);
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					toastr.success(result.msg, COMMON_LABEL_SUCCESS);
					$rootScope.$state.go('prodTypeAreList');
				} else {
					toastr.error(result.msg, COMMON_LABEL_ERROR);
				}
		    }
		}else{ // is add
			var url = "../prodTypeAre/addProdTypeAre";
			jsonPost(url, reqData, callback, true, false);
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					toastr.success(result.msg, COMMON_LABEL_SUCCESS);
	...
	// delete an prodTypeAre
	$scope.toDeleteProdTypeAre = function(){
		if($stateParams.id){
			bootbox.confirm("是否删除?", function (result) {
				if(result){
					var reqData= $stateParams;
					var url = "../prodTypeAre/deleteProdTypeAre";
					jsonGet(url, reqData, callback, true, false);
					function callback(result) {
						if (result.flag == RESULT_FLAG_SUCCESS) {
							toastr.success(result.msg, COMMON_LABEL_SUCCESS);

//list ctrl
angular.module('backApp').controller('higListCtrl', ['$rootScope', '$scope', 'settings', function($rootScope, $scope, $http) {
	
	$scope.$on('$viewContentLoaded', function() {  				
		var higListDataTable = new Datatable().init({
            src: $("#higListDataTable"),
            dataTable: {
    			"language": dtLanguage,
    			...
    			"ajax" : {
    				"type" : "POST",
    				"url" : "../prodTypeHig/getProdTypeHigList", // ajax
    				"contentType": "application/json; charset=utf-8",
    				"data": function (dtRequest) {
    					  dtRequest = $.extend({}, dtRequest, {condition:$scope.higCondition}); // merge the form data into current Query
    			...
    			},
    			{
    				...
    				"render" : function(data, type, row, meta) {
    					var str = '<a class="btn default" href="#/prodTypeHig/form/' + data + '">编辑</a>';
    					return str;
    				},
    				"targets" : 7,
    				'searchable' : false,
	...
	$scope.reloadTable = function(){
		var higListDataTable= $('#higListDataTable').DataTable();
		higListDataTable.ajax.reload();
	}
...
//form ctrl
angular.module('backApp').controller('higFormCtrl', ['$rootScope', '$scope','$stateParams','$state', 'settings', function($rootScope, $scope,$stateParams,$state, $http) {
	// page load: retrieve admin
	$scope.$on('$viewContentLoaded', function() { 
		if($stateParams.id > 0){ // is update
			var reqData= $stateParams;
			var url = "../prodTypeHig/getProdTypeHig";
			jsonGet(url, reqData, callback, true, false);
			
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					$scope.prodTypeHig = result.data;
	...
	// update or add an prodTypeHig
	$scope.toSaveProdTypeHig = function(){

		var reqData = $scope.prodTypeHig;
		...
		if($stateParams.id > 0){ // is update
			var url = "../prodTypeHig/updateProdTypeHig";
			jsonPost(url, reqData, callback, true, false);
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					toastr.success(result.msg, COMMON_LABEL_SUCCESS);
					$rootScope.$state.go('prodTypeHigList');
				} else {
					toastr.error(result.msg, COMMON_LABEL_ERROR);
				}
		    }
		}else{ // is add
			var url = "../prodTypeHig/addProdTypeHig";
			jsonPost(url, reqData, callback, true, false);
			function callback(result) {
				if (result.flag == RESULT_FLAG_SUCCESS) {
					toastr.success(result.msg, COMMON_LABEL_SUCCESS);
	...
	// delete an prodTypeHig
	$scope.toDeleteProdTypeHig = function(){
		if($stateParams.id){
			bootbox.confirm("是否删除?", function (result) {
				if(result){
					var reqData= $stateParams;
					var url = "../prodTypeHig/deleteProdTypeHig";
					jsonGet(url, reqData, callback, true, false);
					function callback(result) {
						if (result.flag == RESULT_FLAG_SUCCESS) {
							toastr.success(result.msg, COMMON_LABEL_SUCCESS);
